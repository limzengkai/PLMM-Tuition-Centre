...
function CardFeeManagementEdit({ color }) {
    ...
    async function fetchStudentsAndUsers() {
      try {
        ...
        if (feeSnapshot.exists()) {
          ...
          const feeClassesData = feeClassesSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
  ...
  const handleEditFee = async () => {
    try {
      const result = await Swal.fire({
        title: 'Are you sure?',
        text: 'You are about to save changes. This action cannot be undone.',
        icon: 'warning',
      ...
      if (result.isConfirmed) {
        // User confirmed, proceed with saving changes
        ...
        for (const feeItem of fees.classes) {
          const existingDoc = snapshot.docs.find(doc => doc.id === feeItem.id);
          
          if (existingDoc) {
            // If the document exists, update it with new fee data
            ...
            // If the document does not exist, add a new document with the corresponding fee data
        ...
        setFormError(""); // Clear the form error if the submission is successful
      }
    } catch (error) {
  ...
  const handleAddFeeItem = () => {
    const newFeeItem = { Descriptions: [""], FeeAmounts: [""], Quantity: [""] }; // Initialize Quantity array
    setFees(prevState => ({
      ...prevState,
      classes: [...prevState.classes, newFeeItem]
    }));
    console.log("Add new fee item");
    console.log(fees);
  };
  ...
  useEffect(() => {
    ...
    console.log("Deleted Fees", deletedFee);
  }, [fees]);
      ...
      {loading ? (
        ...
        <div className="container mx-auto px-4 py-8">
          <div className="flex items-center mb-4 font-bold text-xl">
            ...
            <Link to={`/admin/fee/view/${id}`} className="text-blue-500 hover:underline">View Student's fee</Link>
            <span className="mx-2">&nbsp;/&nbsp;</span>
            <span className="text-gray-500">Edit Student's Fee</span>
          </div>
          <div className="block w-full overflow-x-auto">
            <div className="grid grid-cols-2 gap-y-3 mt-2 m-0 px-8">
              <div className="font-bold mb-4 flex justify-center text-xl col-span-2 underline">User Information</div>
            ...
            <div className="grid grid-cols-2 gap-y-3 mt-2 m-0 px-8">
              <div className="flex justify-center mt-5 mb-4 font-bold text-xl col-span-2 underline">Fee Details</div>
              <div className="font-bold">Due Date: <span className="font-normal">{fees.feeDetail.DueDate.toDate().toDateString()}</span></div>
              <div className="font-bold">Payment Status: <span className="font-normal">{fees.feeDetail.paymentStatus ? "Paid" : "Not Paid"}</span></div>
              <div className="font-bold">Payment Date: <span className="font-normal">{fees.feeDetail.paymentStatus ? fees.feeDetail.paymentDate.toDate().toDateString() : "Not Paid"}</span></div>
            ...
            <table className="mt-5 items-center w-full bg-transparent table-auto border-collapse">
              ...
              <tbody>
              {fees.classes && fees.classes.map((feeItem, index) => (
                <tr key={index}>
                  {/* Render each description */}
                  {feeItem.Descriptions.map((description, idx) => (
                    <td key={idx} className="w-1/2 border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                      <input 
                        type="text"
                        value={description} 
                        ...
                        onChange={(e) => {
                          ...
                          updatedClasses[index].Descriptions[idx] = e.target.value;
                          setFees(prevState => ({ ...prevState, classes: updatedClasses }));
                        }}
                      />
                    </td>
                  ))}
                  
                  {/* Render each fee amount */}
                  {feeItem.FeeAmounts.map((feeAmount, idx) => (
                    <td key={idx} className="w-1/4 border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                      <input 
                        type="number"
                        value={feeAmount}
                        className="border-0 px-3 py-3 border-black placeholder-blueGray-300 text-blueGray-600 rounded text-sm shadow focus:outline-none focus:ring w-full ease-linear transition-all duration-150"
                        ...
                        onChange={(e) => {
                          const updatedClasses = [...fees.classes];
                          updatedClasses[index].FeeAmounts[idx] = e.target.value;
                          setFees(prevState => ({ ...prevState, classes: updatedClasses }));
                        }}
                      />
                    </td>
                  ...
                  {feeItem.Quantity.map((quantity, idx) => (
                    <td key={idx} className="w-1/4 border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                      <input 
                        ...
                        onChange={(e) => {
                          const updatedClasses = [...fees.classes];
                          updatedClasses[index].Quantity[idx] = e.target.value;
                          setFees(prevState => ({ ...prevState, classes: updatedClasses }));
                        }}
                      />
                    </td>
                  ...
                  {feeItem.FeeAmounts.map((feeAmount, idx) => (
                    <td key={idx} className="w-1/4 border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                      {/* Display the calculated amount (Fee * Quantity) */}
                      RM {(parseFloat(feeAmount) * parseFloat(feeItem.Quantity[idx])).toFixed(2)}
                    </td>
                  ))}
            ...
            <div className="font-bold flex justify-center">
              ...
              <span className="font-normal">
                ...
                {fees && fees.classes.reduce((acc, curr) => {
                  // Calculate the total amount for each fee item (Fee * Quantity)
                  const totalAmountPerItem = curr.FeeAmounts.reduce(
                    (total, fee, idx) => total + (parseFloat(fee) * parseFloat(curr.Quantity[idx])), 0);
                  // Add the total amount per item to the accumulator
