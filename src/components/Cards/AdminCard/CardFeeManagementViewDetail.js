        ...
        <div className="container mx-auto px-4 py-8">
          ...
          <div className="block w-full overflow-x-auto">
            <div className="grid grid-cols-2 gap-y-3 mt-2 m-0 px-8">
              <div className="font-bold flex justify-center text-xl col-span-2 underline">User Information</div>
              ...
              <tbody>
                {fees.classes.map((Classfee) => (        
                  <tr key={Classfee.ClassId}>
                    ...
                    {Classfee.Quantity.map((quantity, index) => (
                      <td key={index} className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                        {quantity}
                      </td>
                    ))}
                  {Classfee.FeeAmounts.map((feeAmount, index) => (
                    <td key={index} className="w-1/4 border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                      {/* Display the calculated amount (Fee * Quantity) */}
                      RM {(parseFloat(feeAmount) * parseFloat(Classfee.Quantity[index])).toFixed(2)}
                    </td>
                  ))}
                  </tr>
                ))}
                <tr>
                  ...
                  <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-lg whitespace-nowrap p-4 font-bold">
                    RM {fees.classes.reduce((acc, curr) => acc + curr.FeeAmounts.reduce((subAcc, subCurr) => subAcc + Number(subCurr), 0), 0).toFixed(2)}
                  </td>
                </tr>
              </tbody>
            </table>
